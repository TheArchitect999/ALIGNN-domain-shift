...
REPORTING_HINT = (
    "finetune tagged summarization requires pandas and matplotlib. "
...
def collect_finetune_rows(
    ...
    for family in families:
        for n_value in ns:
            for seed in seeds:
                summary_path = repo / "results/protocol_2" / family / f"N{n_value}_seed{seed}" / run_subdir / "summary.json"
...
def main() -> int:
    ...
    if args.experiment_tag == "finetune":
        ...
        default_out_dir = "results/summaries/protocol_2/finetune"
        default_plot_dir = "results/derived_evidence/protocol_2/Learning Curves"
    else:
        default_run_subdir = f"finetune_last2_{args.experiment_tag}"
        default_out_dir = f"results/derived_evidence/{args.experiment_tag}"
    ...
    if runs_df.empty:
        raise SystemExit("No tagged finetune fine-tuning summaries were found.")
    ...
    manifest_json = out_dir / "finetune_summary_manifest.json"
    ...
    for family in args.families:
        if args.experiment_tag == "finetune":
            plot_stem = f"{family.capitalize()} Learning Curve - protocol_2"
        else:
