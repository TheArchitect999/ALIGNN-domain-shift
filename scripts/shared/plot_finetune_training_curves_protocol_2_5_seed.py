...
DEFAULT_TAG = "finetune"
DEFAULT_NS = [10, 50, 100, 200, 500, 1000]
DEFAULT_SEEDS = [0, 1, 2, 3, 4]
REPORTING_HINT = (
    "finetune tagged training-curve plotting requires numpy and matplotlib. "
    "Bootstrap the Colab environment with "
    "`environment/setup_colab.sh`."
)
...
def load_plot_modules():
    ...
    if missing:
        ...
        raise SystemExit(f"Missing plotting dependencies: {missing_text}. {REPORTING_HINT}")
    return np, plt


def load_history(path: Path, np):
...
def relative_reduction(losses, np):
    if len(losses) == 0:
        return losses
...
def discover_runs(
    ...
    for family in families:
        for n in ns:
            for seed in seeds:
                run_dir = repo_root / "results/protocol_2" / family / f"N{n}_seed{seed}" / run_subdir
                ...
                runs.append(
                    {
                        ...
                        "run_dir": run_dir,
                        "summary": summary,
                        "history_train": load_history(train_path, np),
                        "history_val": load_history(val_path, np),
                    }
                )
    return runs


def plot_run(run: dict, out_dir: Path, np, plt) -> dict:
    family = run["family"]
    n = run["N"]
    seed = run["seed"]
    summary = run["summary"]
    ...
    best_epoch = int(summary.get("best_epoch") or (np.argmin(val) + 1))
    best_val = float(np.min(val))

    train_reduction = relative_reduction(train, np)
    val_reduction = relative_reduction(val, np)
    ...
    fig.text(
        ...
        -0.01,
        (
            "Protocol: pretrained ALIGNN with explicit partial fine-tuning only; "
            "trainable groups = fc + gcn_layers.3; "
...
def plot_family_grid(
    ...
    for row_idx, seed in enumerate(seeds):
        for col_idx, n in enumerate(ns):
            ...
            best_epoch = int(run["summary"].get("best_epoch") or (np.argmin(val) + 1))
    ...
    fig.suptitle(
        f"{family.capitalize()} tagged training-curve grid ({seed_count} seeds x {family_count} N values)",
        fontsize=14,
        fontweight="bold",
    )
...
def write_manifest(out_dir: Path, rows: list[dict], grid_rows: list[dict]) -> None:
    ...
    lines = [
        "# Tagged finetune Training Curves",
        "",
        "These figures were generated from the saved `history_train.json` and `history_val.json` files for every completed tagged finetune fine-tuning run.",
        "",
        "Important note:",
    ...
    (out_dir / "training_curve_manifest.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
...
def main() -> None:
    ...
    if args.experiment_tag == "finetune":
        default_run_subdir = "finetune_last2"
        default_out_dir = "results/derived_evidence/protocol_2/Training Curves/finetune"
    else:
        default_run_subdir = f"finetune_last2_{args.experiment_tag}"
        default_out_dir = f"results/derived_evidence/{args.experiment_tag}/training_curves"
    ...
    if not runs:
        raise SystemExit("No completed tagged fine-tuning runs with history files were found.")

    per_run_rows = [plot_run(run, out_dir, np, plt) for run in runs]
    grid_rows = [
        plot_family_grid(
            runs,
            family,
            out_dir,
            family_count=len(args.Ns),
