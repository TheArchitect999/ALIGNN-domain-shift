...
python scripts/shared/summarize_from_scratch_reports.py \
  ...
  --finetune-results-root results/protocol_1 \
  ...
  --plot-dir "${PLOT_DIR}" \
  --title-label "from_scratch canonical-protocol From-Scratch Comparison" \
  --plot-name-template "{Family} Comparison Plot - protocol_1" \
  --plot-title-template "{Family} Comparison Plot - protocol_1"
...
python scripts/shared/plot_from_scratch_training_curves.py \
  ...
  --title-label "protocol_1 From-Scratch" \
