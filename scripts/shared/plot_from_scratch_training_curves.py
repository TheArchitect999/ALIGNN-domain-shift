...
def discover_runs(
    ...
    for family in families:
        for n_value in ns:
            for seed in seeds:
                run_dir = repo_root / results_root / family / f"N{n_value}_seed{seed}" / run_subdir
...
def plot_run(repo_root: Path, run: dict, out_dir: Path, protocol_note: str) -> dict:
    ...
    title = f"{family.capitalize()} | N={n_value} | seed={seed} | test MAE={summary['test_mae_eV_per_atom']:.5f}"
    fig.suptitle(title, fontsize=13, fontweight="bold")
    fig.text(
        0.5,
        -0.01,
        f"Protocol: {protocol_note}; best val L1 = {best_val:.5f} at epoch {best_epoch}",
        ha="center",
        fontsize=9,
    )
    ...
    stem = f"{family}_N{n_value}_seed{seed}_training_curves"
...
def plot_family_grid(
    ...
    for row_idx, seed in enumerate(seeds):
        for col_idx, n_value in enumerate(ns):
            ax = axes[row_idx, col_idx]
            run = lookup.get((n_value, seed))
            if run is None:
            ...
            ax.set_title(f"N={n_value}, seed={seed}", fontsize=10)
            ax.grid(alpha=0.2)
...
def write_manifest(
    ...
    lines = [
        ...
        "- one PNG and one PDF per from-scratch run",
...
def main() -> None:
    ...
    parser.add_argument("--Ns", nargs="+", type=int, default=[50, 500])
    parser.add_argument("--seeds", nargs="+", type=int, default=[0, 1, 2, 3, 4])
    parser.add_argument("--out-dir", required=True)
    parser.add_argument("--title-label", default="From-Scratch")
    parser.add_argument(
        "--protocol-note",
        default="randomly initialized ALIGNN trained from scratch",
    )
    ...
    if not runs:
        raise SystemExit("No completed from-scratch runs with history files were found.")
